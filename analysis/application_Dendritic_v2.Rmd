...
```
The ELBO peaks at $\epsilon^2 = 3.4e-4$, and the largest ELBO is -154563.1. For $\epsilon^2 = 1.4e-3$, ELBO is -154581.7. For $\epsilon^2 = 1.3e-2$, ELBO is -155000.8. However, we found that adding an $\epsilon^2 \approx 1e-4$ doesn't seem to effectively solve the tight error bar issue. 
